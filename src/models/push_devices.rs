    ...
    pub fn update(&self, conn: &mut PgConnection) -> Result<(), PushDeviceError> {
        diesel::update(push_devices::table.filter(push_devices::id.eq(self.id)))
            .set((
                ...
                push_devices::project_id.eq(self.project_id),
                ...
                push_devices::capability_enc.eq(&self.capability_enc),
                push_devices::notification_public_key_hash.eq(&self.notification_public_key_hash),
